...
namespace app\minipro\middleware;
...
class CheckToken
{
    public function handle($request, \Closure $next)
    {
        ...
        $whiteList = ["testhhk\/","upload\/","MiniUser\/login","index\/"];
        $temp = implode("|",$whiteList);
        $pattern = "/^\/minipro\/".$temp.".*/";
        ...
        if ($matches
            ...
            if (!array_key_exists('status', $jwt_info)) {
                ...
                $request->endtime = $jwt_info['data']->endtime;
                $request->jwtInfo = $jwt_info['data'];

                $current_time = time();
                if($current_time >= $jwt_info['data']->endtime){
                    $re = new ReturnJson(["info"=>$jwt_info,"msg"=>"token过期"],"fail");
                    return json($re->json);
                }
