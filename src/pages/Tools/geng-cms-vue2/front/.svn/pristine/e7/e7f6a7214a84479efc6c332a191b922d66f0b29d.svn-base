...
<script>
import { get_doctor_user_info, update_doctor_user_info } from '@/api/doctor_user_info'
...
export default {
  ...
  methods: {
    ...
    refresh() {
      var that = this
      get_doctor_user_info(that.query).then(res => {
    ...
    confirm() {
      ...
      update_doctor_user_info(temp).then(res => {
        if (res.code === 20000) {
